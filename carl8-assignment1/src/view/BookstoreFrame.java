...
public final class BookstoreFrame extends JFrame {
    ...
    private JPanel makeItemsPanel(final List<Item> theItems) {
        ...
        //System.out.println(  theItems.add(null)  );
        
        p.setBackground(R.Colors.CONTENT_BG);
        for (final Item item : theItems) {               // how does  "theItems" know the size
            addItem(item, p);            //  I think it goes in the first field
           //System.out.println( "*"+item.toString());      why are item's names null?
        }

       //   How does the loop know the size?
